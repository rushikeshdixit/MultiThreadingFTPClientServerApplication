...
public class ControlConnection implements Runnable {
	...
	public void run() {
		...
		try {
			...
			BufferedReader bufferedReader = new BufferedReader(new FileReader("<PathToCredentialsFile>/Credentials.txt"));
			File f=new File("<PathToCredentialsFile>/Credentials.txt");
			...
			linesread=bufferedReader.readLine();
			...
			while(usertoken.equals(details[0]) && password.equals(details[1]))
	            {
	            	allow=true;

	                if(password.matches(details[1])){
	...
	private String checkUser(String command) {
		...
		String userPath = "<YourWorkspacePath>" + username;
